...
class DocumentProcessingToolkit(BaseToolkit):
    ...
    """
    def __init__(self, cache_dir: Optional[str] = None, process_model: Optional[BaseModelBackend] = None):
    ...
    @retry((requests.RequestException,), tries=2, delay=30)
    def extract_document_content(self, document_path: str, query: str) -> Tuple[bool, str]:
        ...
        Args:
            ...
            query (str): The query to be used for retrieving the content. If the content is too long, the query will be used to identify which part contains the relevant information (like RAG). The query should be consistent with the current task. Boolean operators or search-engine-style expressions (e.g., "OR", "AND", "NOT") are not supported, as downstream components rely on direct matching and cannot parse logical query structures.
        ...
        if any(document_path.endswith(ext) for ext in ['txt']):
            with open(document_path, 'r', encoding='utf-8') as f:
                content = f.read()
            f.close()
            res = self._post_process_result(content, query,self.process_model)
            return True, res
        ...
        if self._is_webpage(document_path):
            ...
            #print(a)      
            result_filtered = self._post_process_result(extracted_text, query,self.process_model)
            ...
            try:
                #print("Not webpage")
                result = self._extract_webpage_content(document_path)
                #print(result)  
                ...
                result_filtered = self._post_process_result(result, query,self.process_model)
                return True, result_filtered
...
"""
    ...
    @retry((requests.RequestException,), tries=3, delay=30)
    ...
    @retry((requests.RequestException,), tries=2, delay=60)
    ...
    def _extract_webpage_content(self, url: str) -> str:
        ...
        print("_extract_webpage_content!!!")
        LIMIT = 1
        FORMATS = ["markdown"]
        POLL_INTERVAL = 1
        HARD_TIMEOUT = 60
        ...
        p = mp.Process(
            target=_firecrawl_worker,
            args=(q, url, API_KEY, LIMIT, FORMATS, POLL_INTERVAL),
            daemon=True,   # ✅ 关键：避免 join 阻塞
        )
        p.start()
        ...
        # ✅ 超时或无结果 → fallback
        if got is None:
            logger.warning(f"Firecrawl timeout at {HARD_TIMEOUT}s, using html2text fallback: {url}")
        ...
        if status == "err":
            if any(code in s for code in ["403", "429", "Payment Required"]):
                logger.error(f"Firecrawl error ({s}), fallback to html2text")
                return f"Firecrawl error ({s})"
            raise RuntimeError(s)

        # ✅ 拿到成功结果但 data 为空 → 立即 fallback
        data = payload
        if not data.get("data"):
            logger.debug("Firecrawl success but no markdown content, fallback to html2text.")
            return "Unable to process this webpage."
