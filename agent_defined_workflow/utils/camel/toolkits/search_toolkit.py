...
class SearchToolkit(BaseToolkit):
    ...
    def search_wiki_revisions(
        ...
        Example:
            >>> revisions = search_wiki_revisions("Penguin", 2022, 12)
        ...
        # -------- 必须的 User-Agent（否则 403）--------
        headers = {
            "User-Agent": "AnemoiResearchBot/0.1 (contact: your_email@example.com)"
        }

        # -------- API 参数（不要一次 rvlimit=max）--------
        params = {
            "action": "query",
            "format": "json",
            "titles": entity,
            ...
            "rvend": end_iso,
            "rvdir": "newer",
            "rvlimit": 500  # 稳定值，max + 历史 revision 很容易被挡
        }
            ...
            for page in pages.values():
                for rev in page.get("revisions", []):
                    ...
                    revisions_list.append({
                        "timestamp": timestamp,
                        "oldid": oldid,
                        "url": f"https://en.wikipedia.org/w/index.php?title={entity}&oldid={oldid}"
                    })
