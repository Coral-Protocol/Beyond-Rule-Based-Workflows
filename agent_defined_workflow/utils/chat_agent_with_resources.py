...
class MCPResourceAgent(ChatAgent):
    ...
    def _extract_resource_urls(self, text: str) -> List[str]:
        ...
        pattern = r'<resource>(https?://[^<]+)</resource>'
    ...
    async def _fetch_resource(self, url: str) -> str:
        ...
        try:
            ...
            for uri in uri_list:
                current_uri = uri
                contents_result = await session.read_resource(current_uri)
    ...
    async def _refresh_system_message(self) -> None:
        ...
        for url in resource_urls:
            ...
            replacement = f"<resource>url=\"{url}\">\n{content}\n</resource>"
            updated_content = updated_content = re.sub(pattern, replacement, updated_content, flags=re.DOTALL)
                    ...
                    if response.tool_call_requests:
                        for call in response.tool_call_requests:
                            if (
                                ...
                                and self.agent_name != "information_flow_orchestrator"
                            ):
                                call.args["mentions"] = ["information_flow_orchestrator"]
            ...
            if tool_call_requests := response.tool_call_requests:
                ...
                for tool_call_request in tool_call_requests:
                    if (
                        ...
                        while tool_call_record.tool_name == "wait_for_mentions" and tool_call_record.result == "No new messages received within the timeout period":
